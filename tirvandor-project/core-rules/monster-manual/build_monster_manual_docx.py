...
# Cover page
cover_path = "/home/claude/tirvandor-project/monster-manual/images/monster-manual-cover.png"
if os.path.exists(cover_path):
    try:
        ...
        print(f"⚠️  Cover error: {e}, using text title")
...
for filename, title in chapters:
    ...
    idx = 0
    monster_count = 0  # Track monsters for page breaks
    while idx < len(lines):
        line = lines[idx].strip()
        ...
        # Monster heading detection
        # Pattern: "## 1. MONSTER NAME" or "### 1. MONSTER NAME"
        # NOT: "### LORE" or "### TACTICS" (subsections)
        
        if line.startswith('### '):
            raw_text = line[4:].strip()
            # Check if it's a numbered monster (e.g., "1. MONSTER NAME")
            if re.match(r'^\d+\.', raw_text):
                # This is a monster heading
                raw_name = clean(raw_text)
                display_name = title_case(raw_name)
                
                # Page break before each monster (except first)
                if monster_count > 0:
                    doc.add_page_break()
                monster_count += 1
                ...
                # This is a subsection (LORE, TACTICS, etc.)
                header = clean(raw_text)
        ...
        if line.startswith('## '):
            ...
            # Skip section headers like "## AETHORIA RESISTANCE (8 Creatures)"
            if '(' in raw_text and ('Creatures' in raw_text or 'Monsters' in raw_text):
                p = doc.add_heading(raw_text, 2)
                for run in p.runs:
                    run.font.color.rgb = RGBColor(70, 130, 180)
                idx += 1
                continue
            
            # This is a numbered monster
            raw_name = clean(raw_text)
            display_name = title_case(raw_name)
