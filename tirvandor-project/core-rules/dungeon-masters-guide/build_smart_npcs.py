#!/usr/bin/env python3
"""
Build smart NPC stat blocks:
- One table for all stats
- Reactions only if appropriate
- Legendary actions only for major NPCs (CR 10+)
- Roleplaying notes based on character
"""
...
print("🧙 Building smart NPC stat blocks...")
...
# Helper: Determine if NPC should have reactions
def should_have_reactions(npc):
    name_lower = npc['name'].lower()
    # Spellcasters, high-level fighters, royalty
...
# Helper: Determine if NPC should have legendary actions
...
# Helper: Add default reactions if missing
...
# Helper: Add default legendary actions if missing
def add_default_legendary(npc):
    if not npc['legendary'] and should_have_legendary(npc):
        ...
        # Add class-specific legendary action
        name_lower = npc['name'].lower()
        if 'mage' in name_lower or 'wizard' in name_lower:
...
# Helper: Add roleplaying notes if missing
...
doc.add_heading("NPC Stat Blocks - Test Sample", 1)
doc.add_paragraph("Testing smart stat block format with 10 NPCs")
doc.add_paragraph()

# Process first 10 NPCs
for npc in npcs[:10]:
    # Update NPC with smart defaults
    ...
    info_parts = []
    if npc['saves']:
        info_parts.append(f"**Saves** {npc['saves']}")
    if npc['skills']:
        info_parts.append(f"**Skills** {npc['skills']}")
    if npc['senses']:
        info_parts.append(f"**Senses** {npc['senses']}")
    if npc['languages']:
        info_parts.append(f"**Languages** {npc['languages']}")
    ...
    if npc['actions']:
        ...
        # Add default action
        doc.add_paragraph("**Unarmed Strike.** *Melee Weapon Attack:* +2 to hit, reach 5 ft., one target. *Hit:* 2 (1 + 1) bludgeoning damage.", style='List Bullet')
    doc.add_paragraph()
    ...
    # ROLEPLAYING NOTES (always present)
    p = doc.add_paragraph()
...
# Save
output = '/mnt/user-data/outputs/Smart-NPC-Stat-Blocks.docx'
doc.save(output)

print(f"\n✅ Created smart NPC stat blocks")
print(f"File: {output}")
print(f"Size: {os.path.getsize(output) / 1024 / 1024:.1f} MB")
print("\n✅ Smart features:")
print("  - One complete stat table per NPC")
print("  - Reactions only for appropriate NPCs (CR 5+, spellcasters)")
print("  - Legendary actions only for major NPCs (CR 8+, bosses)")
print("  - Roleplaying notes match character role")
print("  - All NPCs have actions (default if missing)")
