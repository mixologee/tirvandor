...
print("Building Final DMG with Maps...")

doc = Document()

# Cover (NO legal pages at start)
try:
...
toc = [
    ...
    ("  Appendix D: Battle Maps (20 maps)", "485"),
    ("  Appendix E: Region Maps (6 maps)", "495"),
    ("", ""),
    ("Legal", ""),
    ("  Copyright Notice", "505"),
    ("  Attribution", "506"),
    ("  Open Game License", "507"),
]
...
for ch in ["01-introduction-dm-secrets.md", "02-running-the-game.md", "03-adventure-creation.md",
           "04-campaign-creation.md", "05-secret-locations.md", "06-magic-mechanics.md",
           "11-treasure-comprehensive.md"]:
    process_md(f'markdown/{ch}', doc)
    doc.add_page_break()

# NPCs (abbreviated for space)
...
npcs, current, section = [], None, None
...
for npc in npcs:
    ...
    p = doc.add_paragraph()
    p.add_run(npc['type'] or "Medium humanoid").italic = True
...
# BATTLE MAPS
print("Adding battle maps...")
doc.add_heading("Appendix D: Battle Maps", 1)
doc.add_paragraph("Tactical battle maps for encounters")
doc.add_paragraph()

battle_maps = [
    'images/maps/border-wraith-haunting-ground.jpg',
    'images/maps/ironhold-behemoth-cave.jpg',
    'images/maps/corruption-spawn-nest.jpg',
    'images/maps/ley-line-elemental-node.jpg',
    'images/maps/ancient-guardian-chamber.jpg',
    'images/maps/void-wraith-den.jpg',
]

# Add campaign maps too
campaign_maps = [
    '../../../blood-and-coin/images/maps/warehouse-district-ambush-day.jpg',
    '../../../blood-and-coin/images/maps/goldreach-sewers.jpg',
    '../../../blood-and-coin/images/maps/dockside-smuggler-den.jpg',
    '../../../blood-and-coin/images/maps/final-showdown-arena.jpg',
    '../../../blood-and-coin/images/maps/broken-spear-tavern.jpg',
    '../../../blood-and-coin/images/maps/iron-guild-hall.jpg',
    '../../../blood-and-coin/images/maps/criminal-safehouse.jpg',
]

map_count = 0
for bmap in battle_maps + campaign_maps:
    if os.path.exists(bmap):
        try:
            doc.add_heading(os.path.basename(bmap).replace('-', ' ').replace('.jpg', '').title(), 3)
            doc.add_picture(bmap, width=Inches(6.5))
            doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
            doc.add_paragraph()
            map_count += 1
        except: pass

print(f"  Added {map_count} battle maps")

# REGION MAPS
doc.add_page_break()
doc.add_heading("Appendix E: Region Maps", 1)
doc.add_paragraph("World and regional maps of Tirvandor")
doc.add_paragraph()

region_maps = [
    '../world-guide/images/world-maps/tirvandor_map_locations.jpg',
    '../world-guide/images/world-maps/tirvandor_map_regions.jpg',
    '../world-guide/images/world-maps/tirvandor_map_ley_lines.jpg',
    '../world-guide/images/world-maps/tirvandor_map_locations_thaldros.jpg',
    '../world-guide/images/world-maps/tirvandor_map_locations_aethoria.jpg',
    '../world-guide/images/world-maps/tirvandor_map_locations_sundering_sea.jpg',
]
...
for rmap in region_maps:
    if os.path.exists(rmap):
        try:
            doc.add_heading(os.path.basename(rmap).replace('_', ' ').replace('.jpg', '').title(), 3)
            doc.add_picture(rmap, width=Inches(6.5))
            doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
            doc.add_paragraph()
            region_count += 1
        except: pass

print(f"  Added {region_count} region maps")
...
doc.add_page_break()
doc.add_heading("Attribution", 1)
doc.add_paragraph("This work includes material taken from the System Reference Document 5.1 (SRD 5.1) by Wizards of the Coast LLC.")
doc.add_paragraph()
...
doc.add_paragraph("The following are designated as Product Identity: Tirvandor, all proper names, storylines, original artwork and maps, the Seven Primordials concept, all original magic items, the Sundering event and timeline, all campaign content.")

doc.add_page_break()
doc.add_heading("OPEN GAME LICENSE Version 1.0a", 1)
ogl = """The following text is the property of Wizards of the Coast, Inc. and is Copyright 2000 Wizards of the Coast, Inc ("Wizards"). All Rights Reserved.
...
"""
for para in ogl.split('\n\n'):
    doc.add_paragraph(para)

# Save
out = '/mnt/user-data/outputs/Tirvandor-DMG-Complete-Final.docx'
doc.save(out)
...
print(f"Battle Maps: {map_count}")
print(f"Region Maps: {region_count}")
print(f"✅ Legal pages at END")
