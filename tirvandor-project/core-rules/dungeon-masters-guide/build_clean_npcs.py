#!/usr/bin/env python3
"""
Build clean NPC stat blocks - NO separator, just clean end
"""
...
print("🧙 Building clean NPC stat blocks (no separator)...")
...
doc.add_heading("Clean NPC Stat Blocks - Test", 1)
doc.add_paragraph("No separator - just clean end after roleplaying notes")
doc.add_paragraph()
...
for npc in npcs[:10]:
    ...
    # ROLEPLAYING NOTES - clean end, no separator
    p = doc.add_paragraph()
    ...
    # That's it! Next NPC heading will separate them naturally

# Save
output = '/mnt/user-data/outputs/Clean-NPC-Stat-Blocks.docx'
doc.save(output)

print(f"\n✅ Created clean NPC stat blocks")
print(f"File: {output}")
print(f"Size: {os.path.getsize(output) / 1024 / 1024:.1f} MB")
print("\n✅ PERFECT: No separator, no blank lines")
print("   NPCs flow naturally with heading separation")
