#!/usr/bin/env python3
"""
Build polished NPC stat blocks with final formatting fixes
"""
...
print("🧙 Building polished NPC stat blocks...")

# Read NPC file
with open('markdown/07-npc-deep-lore.md', 'r', encoding='utf-8') as f:
    content = f.read()

# Parse NPCs (same as before)
npcs = []
current_npc = None
...
def expand_save_abbreviations(saves_text):
    """Convert abbreviated saves to full words"""
    if not saves_text:
        return saves_text
...
def add_formatted_text(paragraph, text):
    """Add text with proper bold formatting, no markdown artifacts"""
    # Split on ** markers for bold
...
doc.add_heading("Polished NPC Stat Blocks - Test", 1)
doc.add_paragraph("Final formatting test with 10 NPCs")
doc.add_paragraph()
...
for npc in npcs[:10]:
    ...
    # Separate lines for Saves, Senses, Languages - with bold labels
    if npc['saves']:
        p = doc.add_paragraph()
    ...
    # NO blank line at end - separator only
    doc.add_paragraph("─" * 80)

# Save
output = '/mnt/user-data/outputs/Polished-NPC-Stat-Blocks.docx'
doc.save(output)

print(f"\n✅ Created polished NPC stat blocks")
print(f"File: {output}")
print(f"Size: {os.path.getsize(output) / 1024 / 1024:.1f} MB")
print("\n✅ Final polish:")
print("  - No blank line at end of entries")
print("  - Saves/Skills/Senses/Languages on separate lines")
print("  - Bold labels (not markdown **)")
print("  - Full save names (Wisdom not Wis)")
print("  - Proper bold formatting in text")
